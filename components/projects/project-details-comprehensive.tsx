...
export function ProjectDetailsComprehensive({ projectId }: { projectId: string }) {
  ...
  // Use detailedTonsPerLatestRev if available, otherwise use detailedTonsPerApproval as fallback
  const totalTons = data.detailedTonsPerLatestRev ?? data.detailedTonsPerApproval ?? null;
  let yetToReleaseTons: number | null = null;
  
  if (totalTons != null) {
    if (data.releasedTons != null) {
      yetToReleaseTons = Math.max(0, totalTons - data.releasedTons); // Ensure non-negative
    } else {
