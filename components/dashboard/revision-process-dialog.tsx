...
export function RevisionProcessDialog({
  ...
  const revisionProjects = React.useMemo(() => {
    return projects.filter((p) => p.revisionStatus === "IN PROCESS");
  }, [projects]);
  ...
  const paginatedProjects = revisionProjects.slice(startIndex, endIndex);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-screen-xl w-full min-w-[95vw] max-h-[90vh] h-auto p-0 flex flex-col">
        <DialogHeader className="px-6 pt-6 pb-4 shrink-0 w-full border-b">
          ...
          <DialogDescription>
            List of all projects currently in revision process
          </DialogDescription>
        </DialogHeader>
