...
export function ReleasedJobsDialog({
  ...
  const releasedProjects = React.useMemo(() => {
    return projects.filter((p) => p.releaseStatus?.includes("RELEASED"));
  }, [projects]);

  // Calculate pagination
  const total = releasedProjects.length;
  const totalPages = Math.ceil(total / pageSize);
  const startIndex = (page - 1) * pageSize;
  const endIndex = startIndex + pageSize;
  const paginatedProjects = releasedProjects.slice(startIndex, endIndex);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-screen-xl w-full min-w-[95vw] max-h-[90vh] h-auto p-0 flex flex-col">
        <DialogHeader className="px-6 pt-6 pb-4 shrink-0 w-full border-b">
          <DialogTitle>Released in Progress</DialogTitle>
          <DialogDescription>
            List of all released projects and their current status
          </DialogDescription>
        </DialogHeader>
          ...
          {releasedProjects.length === 0 ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-lg font-medium">No released projects in progress</p>
