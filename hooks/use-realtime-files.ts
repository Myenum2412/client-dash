...
export function useRealtimeFiles(
  ...
  const fetchFiles = useCallback(async (isBackgroundRefresh = false) => {
    ...
    try {
      ...
      const hasChanged = JSON.stringify(files) !== JSON.stringify(newFiles);
      ...
      if (hasChanged) {
        ...
        if (isBackgroundRefresh && typeof window !== 'undefined') {
          console.log(`🔄 Files updated: ${newFiles.length} items loaded`);
        }
      }
